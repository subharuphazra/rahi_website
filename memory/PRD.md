...
3. **Breaking news CRUD** — Ticker at the top now pulls from `/api/breaking`. Admin manages headlines at `/admin/breaking` (add/edit/delete/active-toggle, bilingual, optional link, order). Falls back to latest articles when no items are configured.
